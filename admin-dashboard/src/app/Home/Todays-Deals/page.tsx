...
export default function TodaysDealsPage() {
    ...
    const confirmOfferPrice = () => {
        ...
        setTodaysDeals(prev => [...prev, updatedProduct]);
    ...
    const removeFromDeals = (id: string) => {
        const productToRemove = todaysDeals.find(p => p.id === id);
        if (productToRemove) {
            setProducts(prev => [...prev, productToRemove]);
            setTodaysDeals(prev => prev.filter(p => p.id !== id));
        }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 py-4 rounded-lg shadow-sm">
                    <h2 className="text-[14px] font-semibold mb-4">
                        Todays Deals ({todaysDeals.length})
                    </h2>
